...
subroutine calculos
	...
	do ii=1,lfa
	  do k=1,ng		! grid
		if(grid(ii).eq.grid2(k).and. capa(ii).eq.layer(k)) then
			do i=1,ns  !profiles
...
!
  ...
  do i=1,lfa-1
   do j=i+1,lfa
   if(grid(j).eq.grid(i).and.xl(j).and. capa(i).eq.capa(j)) xl(j)=.false.
   end do
  end do
  j=0
  do i=1,lfa
