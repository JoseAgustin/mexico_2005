...
!	
!
!  Creado por Jose Agustin Garcia Reynoso el 02/06/12.
!
!
!  Proposito:
...
!
! ifort -O2 -axAVX -lnetcdff  guarda2_nc2005.f90 -o radm2.exe
!
...
module vars
   ...
   'E_CO   ','E_NH3  ','E_NO   ', &
    ...
    'SO2             ','ALDEHYDES       ','METHANE         ', &
    'CRESOL          ','Ethane          ','Glyoxal         ', &
    'HC3             ','HC5             ','HC8             ', &
    'HCHO            ','ISOPRENE        ','Acetone         ', &
    'Acrolein        ','MGLY            ','Methyl Vinil Ket', &
    'Alkenes         ','alkenes         ','Terminal Alkynes', &
    'Formic Acid     ','Acetic Acid     ','TOLUENE         ', &
    'XYLENE          ','PM_10           ','PM_25           ', &
    'Sulfates        ','Nitrates        ','PM25I           ', &
    'Organic         ','Elemental Carbon','SulfatesJ       ', &
    'NitratesJ       ','PM25J           ','Organic         ', &
    'Elemental Carbon' /)
...
contains
subroutine lee
	integer :: ii,i,j,k,levl,ih
	integer :: iunit=14
	real ::rdum
	real,dimension(nh)::edum
	real,dimension(nf)::wtm
	character(len=46) :: description
	character(len=13) cdum
	character(len=17),dimension(nf):: fnameA,fnameM,fnameP
	data fnameA /'TACO__2005.txt','TANH3_2005.txt','TANOx_2005.txt','TASO2_2005.txt',&
	...
	& 'RADM-2_XYL_A.txt','TAPM102005.txt','TAPM2_2005.txt', &
	& 'GSO4_A.txt','PNO3_A.txt','OTHE_M.txt','POA_A.txt','PEC_A.txt'/
	data fnameM /'TMCO__2005.txt','TMNH3_2005.txt','TMNOx_2005.txt','TMSO2_2005.txt',&
	...
	& 'RADM-2_XYL_M.txt','TMPM102005.txt','TMPM2_2005.txt', &
		 ...
		 busca: do j=1,ny
		  do i=1,nx
			...
			if(idcg(k).eq.idcf) then
			  do ih=1,nh
				eft(i,j,ii,ih,1)=eft(i,j,ii,ih,1)+edum(ih)/WTM(ii) ! Emission from kg to gmol
              end do
			  exit busca
			end if
		 end do
		 ...
		 busca3: do j=1,ny
		  do i=1,nx
			...
			if(idcg(k).eq.idcf) then
			  do ih=1,nh
                ! Emission from g to gmol by 1/WTM
				eft(i,j,ii,ih,9-levl)=eft(i,j,ii,ih,9-levl)+edum(ih)/WTM(ii)
			  end do
              zlev =max(zlev,levl)
			  exit busca3
			end if
     ...
     do periodo=1,1!12
	  if(periodo.eq.1) then
       ...
       write(iTime(12:13),'(I2)') iit
        FILE_NAME='wrfchemi.d01.radm2.'//iTime
	 end if
...
!     Define dimensiones
	...
	call check( nf90_put_att(ncid, NF90_GLOBAL, "TITLE","EI 2005 emissions for Mexico Area"))
	...
	call check( nf90_put_att(ncid, NF90_GLOBAL, "MOAD_CEN_LAT",20.83480871))
	call check( nf90_put_att(ncid, NF90_GLOBAL, "STAND_LON",-103.6132743))
	...
	call check( nf90_put_att(ncid, NF90_GLOBAL, "JULYR",2005))
