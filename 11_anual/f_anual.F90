...
integer, parameter::nsp=6 !number of pollutants
...
data casn /'MEX_POINT_9km_2005_PM10.nc','MEX_POINT_9km_2005_PM25.nc',&
'MEX_POINT_9km_2005_SO2.nc ','MEX_POINT_9km_2005_CO.nc  ',&
'MEX_POINT_9km_2005_NOx.nc ','MEX_POINT_9km_2005_VOC.nc '/
...
data ename/ 'PM10','PM25','SO2 ','CO  ','NOx ','VOC '/
data cname / &
...
!$omp section
	open(newunit=iun,file='../07_puntual/puntual.csv',status='old',action='read')
	i=0
	read (iun,*) cdum
	do
...
!$omp end parallel sections
    ...
    do i=1,nl
      read(iun,*,err=110)lat(i),lon(i),iscc(i),(e_mis(i,j),j=1,nsp),capa(i,1)
      capa(i,2)=capa(i,1)
    end do
    close(iun)
...
!    print *,"Globales"
    ...
    call check( nf90_put_att(ncid, NF90_GLOBAL, "id","PE_2005_"//trim(ename(k))))
